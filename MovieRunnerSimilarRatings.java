...
public class MovieRunnerSimilarRatings {
    ...
    public void printSimilarRatings() {
        ...
        ArrayList<Rating> ratingList = tr.getSimilarRatings("71", 20, 5);
    ...
    public void printSimilarRatingsByGenre() {
        ...
        ArrayList<Rating> ratingList = tr4.getSimilarRatingsByFilter("964", 20, 5, new GenreFilter("Mystery"));
        System.out.println("Found ratings for movies : " + ratingList.size());
        for (int i = 0; i < 3; i++) {
    ...
    public void printSimilarRatingsByDirector() {
        FourthRatings tr4 = new FourthRatings();
        ArrayList<Rating> ratingList = tr4.getSimilarRatingsByFilter("120", 10, 2, new DirectorsFilter("Clint Eastwood,J.J. Abrams,Alfred Hitchcock,Sydney Pollack,David Cronenberg,Oliver Stone,Mike Leigh"));
        System.out.println("Found ratings for movies : " + ratingList.size());
    ...
    public void printSimilarRatingsByGenreAndMinutes() {
        ...
        a.addFilter(new GenreFilter("Drama"));
        a.addFilter(new MinutesFilter(80, 160));
        ArrayList<Rating> ratingList = tr4.getSimilarRatingsByFilter("168", 10, 3, a);
        System.out.println("Found ratings for movies : " + ratingList.size());
        int printNum = ratingList.size();
        if (printNum >= 3) printNum = 3;
        for (int i = 0; i < printNum; i++) {
    ...
    public void printSimilarRatingsByYearAfterAndMinutes() {
        FourthRatings tr4 = new FourthRatings();//do i need put filename here?
        AllFilters a = new AllFilters();
        a.addFilter(new YearAfterFilter(1975));
        a.addFilter(new MinutesFilter(70, 200));
        ArrayList<Rating> ratingList = tr4.getSimilarRatingsByFilter("314", 10, 5, a);
        System.out.println("Found ratings for movies : " + ratingList.size());
        int printNum = ratingList.size();
        if (printNum >= 3) printNum = 3;
        for (int i = 0; i < printNum; i++) {
